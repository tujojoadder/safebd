            ...
            <table class="padding text-left small border-bottom">
                <thead>
                    <tr class="gry-color" style="background: #eceff4;">
                        ...
                        {{-- <th width="35%" class="text-left">Product Point</th> --}}
                    ...
                    @foreach ($order->order_details as $key => $orderDetail)
                        @if ($orderDetail->product != null)
                            <tr class="">
                                ...
                                <td class="currency">
                                    {{ number_format($orderDetail->price / $orderDetail->qty ?? '0', 2) }}</td>
                                <td class="text-right currency">
                                    {{ number_format($orderDetail->qty * $orderDetail->price ?? '0', 2) }}</td>
                                @php
                                ...
                                <tbody>
                                    @if ($order->order_type == 1)
                                        <tr>
                                            ...
                                            <td class="currency"> {{ number_format($order->grand_total ?? '0', 2) }}
                                            </td>
                                        </tr>
                                        <tr>
                                            <th class="gry-color text-left">Shipping Cost</th>
                                            <td class="currency">{{ number_format($order->shipping_cost ?? '0', 2) }}
                                            </td>
                                        </tr>
                                    ...
                                    @elseif($order->order_type == 2)
                                        ...
                                        <tr>
                                            <th class="gry-color text-left">Sub Total</th>
                                            <td class="currency">{{ number_format($amount ?? '0', 2) }}</td>
                                        </tr>
                                        <tr>
                                            <th class="gry-color text-left">Shipping Cost</th>
                                            <td class="currency">{{ number_format($order->shipping_cost ?? '0', 2) }}
